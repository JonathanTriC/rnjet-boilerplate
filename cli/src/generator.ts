...
export async function generateProject(answers: any) {
	...
	console.log(chalk.cyan("Injecting src folder structure..."));
	...
	fs.writeFileSync(
		path.join(targetDir, "src/api/apiType.d.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/api/apiUtils.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/api/clientWithoutToken.ts"),
		`import { BASE_URL } from '@constants/url';
import axios, { CreateAxiosDefaults } from 'axios';
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/api/apiWrapping.ts"),
...
`,
	...
	const navCommonStackDir = path.join(
		targetDir,
		"src/navigation/navigator/stack/common-stack",
	);
	const navMainStackDir = path.join(
		targetDir,
		"src/navigation/navigator/stack/main-stack",
	);
	const navStackDir = path.join(targetDir, "src/navigation/navigator/stack");
	const navNavigatorDir = path.join(targetDir, "src/navigation/navigator");
	const navRouteAppDir = path.join(targetDir, "src/navigation/route-app");

	fs.mkdirSync(navCommonStackDir, { recursive: true });
	fs.mkdirSync(navMainStackDir, { recursive: true });
	fs.mkdirSync(navRouteAppDir, { recursive: true });

	// 1. common-stack
	fs.writeFileSync(
		path.join(navCommonStackDir, "CommonStack.tsx"),
...
`,
	...
	fs.writeFileSync(
		path.join(navCommonStackDir, "CommonStackParamList.d.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(navCommonStackDir, "index.ts"),
		`export * from './CommonStack';\n`,
	);

	// 2. main-stack
	fs.writeFileSync(
		path.join(navMainStackDir, "MainStack.tsx"),
		`import { createStackNavigator } from '@react-navigation/stack';
import { useNavigator } from '@navigation/navigator';
...
`,
	...
	fs.writeFileSync(
		path.join(navMainStackDir, "MainStackParamList.d.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(navMainStackDir, "index.ts"),
		`export * from './MainStack';\n`,
	);

	fs.writeFileSync(
		path.join(navStackDir, "index.ts"),
		`export * from './common-stack';\nexport * from './main-stack';\n`,
	);

	// navigator outer files
	fs.writeFileSync(
		path.join(navNavigatorDir, "index.ts"),
		`export * from './Navigator';
export * from './useNavigator';
`,
	);
	fs.writeFileSync(
		path.join(navNavigatorDir, "Navigator.tsx"),
		`import { createStackNavigator } from '@react-navigation/stack';
...
`,
	...
	fs.writeFileSync(
		path.join(navNavigatorDir, "screen.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(navNavigatorDir, "type.d.ts"),
		`type IRouteRef = {
  screenName: string;
  screenStack: string[];
};
`,
	...
	fs.writeFileSync(
		path.join(navNavigatorDir, "useNavigator.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(navRouteAppDir, "index.ts"),
		`export * from './RouteApp';\n`,
	);
	fs.writeFileSync(
		path.join(navRouteAppDir, "styles.ts"),
		`import { StyleSheet } from 'react-native';
...
`,
	...
	fs.writeFileSync(
		path.join(navRouteAppDir, "RouteApp.tsx"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/navigation/index.ts"),
		`export * from './navigator';\nexport * from './route-app';\n`,
	);

	const mainDir = path.join(targetDir, "src/modules/main");
	const homeDir = path.join(mainDir, "home");
	fs.mkdirSync(homeDir, { recursive: true });

	fs.writeFileSync(
		path.join(homeDir, "styles.ts"),
		`import { StyleSheet } from 'react-native';
...
`,
	...
	fs.writeFileSync(
		path.join(homeDir, "useHomeScreen.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(homeDir, "index.ts"),
		`export * from './HomeScreen';\n`,
	);
	fs.writeFileSync(path.join(mainDir, "index.ts"), `export * from './home';\n`);
	fs.writeFileSync(
	...
	fs.writeFileSync(
		path.join(targetDir, "src/theme/light.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/theme/dark.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/theme/text-theme.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/theme/index.ts"),
		`export * from './light';
export * from './dark';
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/hooks/useTheme.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/hooks/index.ts"),
		`export * from './useTheme';\nexport *from './i18n-hooks';\nexport * from './navigation-hooks';\n`,
	);

	const i18nHooksDir = path.join(targetDir, "src/hooks/i18n-hooks");
	fs.mkdirSync(i18nHooksDir, { recursive: true });

	fs.writeFileSync(
		path.join(i18nHooksDir, "i18n.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(i18nHooksDir, "useTypedTranslation.ts"),
		`import {
  useTranslation,
...
`,
	...
	fs.writeFileSync(
		path.join(i18nHooksDir, "index.ts"),
		`export * from './i18n';
    export * from './useTypedTranslation';
`,
	);

	const navHooksDir = path.join(targetDir, "src/hooks/navigation-hooks");
	fs.mkdirSync(navHooksDir, { recursive: true });

	fs.writeFileSync(
		path.join(navHooksDir, "global.d.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(navHooksDir, "index.ts"),
		`export * from './useNavigation';\n`,
	);

	fs.writeFileSync(
		path.join(navHooksDir, "useNavigation.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/constants/functional.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/constants/globalStyles.ts"),
		`import { StyleSheet } from 'react-native';
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/constants/index.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/constants/keys.ts"),
		`export const Keys = {};\n`,
	);

	fs.writeFileSync(
		path.join(targetDir, "src/constants/url.ts"),
		`import Config from 'react-native-config';
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/assets/fonts/Font.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/assets/fonts/index.ts"),
		`export * from './Font';\n`,
	);

	const welcomeDir = path.join(targetDir, "src/i18n/welcome");
	fs.mkdirSync(welcomeDir, { recursive: true });
	fs.writeFileSync(
		path.join(welcomeDir, "id.json"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/i18n/index.ts"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/components/text/Text.tsx"),
...
`,
	...
	fs.writeFileSync(
		path.join(targetDir, "src/components/text/index.ts"),
		`export * from './Text';`,
	);

	fs.writeFileSync(
		path.join(targetDir, "src/components/index.ts"),
		`export * from './text';\n`,
	);
	...
	const textTypeDir = path.join("src/types", "text-type");
	fs.mkdirSync(textTypeDir, { recursive: true });
	fs.writeFileSync(
		path.join(textTypeDir, "index.ts"),
		"export * from './TextType';\n",
	);
	fs.writeFileSync(
		path.join(textTypeDir, "TextType.ts"),
...
`;
	...
	// React SplashScreen
	const splashDir = path.join(targetDir, "src/modules/common/splash");
...
`;
	fs.writeFileSync(path.join(targetDir, "Makefile"), makefileContent);

	fs.writeFileSync(
		path.join(targetDir, "react-native.config.js"),
		`module.exports = {
  project: {
    ios: {},
    android: {},
